# 鼠鼠星球

一个给"已经去鼠星的鼠鼠"做纪念档案的小网站。长期愿景是收录各个玩家心爱的鼠鼠，让每一位已经抵达鼠星的小居民都能被温柔记住。

当前阶段的定位是：轻量网站 + MySQL 数据库存储 + Redis 缓存的鼠鼠纪念档案。
不做账号系统，流程就是：玩家在页面投稿 → 管理员审核 → 审核通过后进入公开纪念星河。
浏览器本地保存只是临时草稿（换设备就没了），不充当"私密档案"。

## 当前边界

- 不做账号系统，没有"我的档案"概念
- 浏览器本地保存仅作临时草稿，方便随手记录
- 玩家投稿进入 MySQL，默认待审核状态
- 管理员审核通过后，档案进入公开纪念星河
- 公开展示只读取审核通过的数据库档案
- 暂不做评论、关注、排行等强社区功能

更完整的产品路线见 [`ROADMAP.md`](./ROADMAP.md)。

## 本地运行

数据库模式需要先安装依赖、配置环境变量、初始化 MySQL，再启动网站服务。

```bash
npm install
copy .env.example .env
npm run db:init
npm run dev
```

启动后访问：

```text
http://127.0.0.1:4173/
```

`.env` 里需要配置：

```text
DB_HOST=43.143.131.87
DB_PORT=3306
DB_USER=xxxzh
DB_PASSWORD=你的数据库密码
DB_NAME=omwstar
REDIS_URL=redis://127.0.0.1:6379
ADMIN_TOKEN=换成一段很长的审核口令
```

`.env` 已加入 `.gitignore`，不要提交真实密码。

如果只想看静态页面，也可以直接打开 `index.html`。这种模式下会显示示例数据和浏览器本地保存内容，但不会连接 MySQL 或 Redis。

页面入口：

- 首页：`index.html`
- 星球居民：`residents.html`
- 鼠鼠纪念页：`resident.html?id=1`
- 星球地图：`map.html`
- 纪念星河：`planet-wall.html`
- 投稿：`submit.html`

当前版本包含：

- 星球首页
- 鼠鼠纪念卡片
- 搜索与区域筛选
- MySQL 数据库存储
- Redis 公开居民列表缓存
- 数据库初始化脚本
- 玩家投稿进入待审核数据库
- 管理审核 API
- 完整本地纪念卡表单
- 编辑本地新增纪念卡
- 删除单张本地新增纪念卡
- 浏览器本地保存新增鼠鼠
- 鼠鼠详情页视图
- 单只鼠鼠独立纪念页
- 复制单只鼠鼠分享文案
- 生成单只鼠鼠纪念卡 PNG 图片
- 鼠鼠星球地图
- 纪念星河公开展示页
- 本地点灯轻互动原型
- 玩家投稿流程静态原型
- 投稿草稿待审核预览
- 可替换的首页主视觉资产
- 顶部分类导航：首页、星球居民、星球地图、纪念星河、投稿
- 独立页面：首页 `index.html`、星球居民 `residents.html`、鼠鼠纪念页 `resident.html?id=1`、星球地图 `map.html`、纪念星河 `planet-wall.html`、投稿 `submit.html`

## 下一步重点

优先把项目打磨成一个完整的个人档案工具：

- 统一初始数据来源，避免重复维护
- 继续优化更完整的纪念字段
- 强化本地编辑、删除和清空确认体验
- 支持导入和导出纪念档案 JSON
- 增加本地保存、导入导出、删除等操作提示
- 为未来公开分享预留 `visibility`、`createdAt`、`updatedAt` 等字段
- 继续完善投稿页面的公开确认和待审核状态

## 未来可能方向

- 生成单张纪念卡图片
- 复制分享文案
- 打印或保存详情页
- 完善投稿审核后台
- 继续完善"纪念星河"的公开展示

## 暂不计划

- 默认公开用户内容
- 评论区
- 关注关系
- 排行榜或热度榜
- 未审核公开投稿
- 过早引入账号系统

## 版本备份

- `shushu-planet-v1.0.1`：继续开发前的备份版本
- `shushu-planet`：当前工作版本
